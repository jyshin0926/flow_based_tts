...
from text import text_to_sequence, cleaned_text_to_sequence, KoG2p
...
class TextAudioLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_and_text, hparams):
        ...
        self._save_dir = "data/jaeyoung/speech_synthesis/VITS/studio_spec"
    ...
    def get_audio_text_pair(self, audiopath_and_text):
        ...
        # TODO:: 향후에 'audiopath', 'text' 이런 식으로 키값으로 받아오는 걸로 변경하기
    ...
    def get_audio(self, filename):
        ...
        spec_filename = os.path.join(self._save_dir, f"{_file}.spec.pt")
...
class TextAudioSpeakerLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, audiopaths_sid_text, hparams):
        self._stage = audiopaths_sid_text["stage"]
        self.audiopaths_sid_text = load_filepaths_and_text(audiopaths_sid_text)

        self._g2p = KoG2p()
        self._cleaner = KoreanCleaner()

        # TODO:: korean_cleaner로 변경?
        self._save_dir = "/data/jaeyoung/speech_synthesis/VITS/studio_spec"
        # self.text_cleaners = hparams.text_cleaners
        self.max_wav_value = hparams.max_wav_value
        self.sampling_rate = hparams.sampling_rate
    ...
    def _filter(self):
        # TODO:: 미리 extract 해두고 load_filepaths_and_text 함수에서 같이 로드하기
        """
        Filter text & store spec lengths
        """
        # Store spectrogram lengths for Bucketing
        ...
        audio_sid_txt_pkl = f"{pkl_path}/audiopaths_sid_text_new_{self._stage}.pkl"
        len_pkl = f"{pkl_path}/lengths_{self._stage}.pkl"
        # audio_sid_txt_pkl = f'{pkl_path}/audiopaths_sid_text_new.pkl'
    ...
    def get_audio_text_speaker_pair(self, audiopath_sid_text):
        ...
        # TODO:: 향후에 'audiopath', 'sid', 'text' 이런 식으로 키값으로 받아오는 걸로 변경하기
    ...
    def get_audio(self, filename, sid):
        audio, sampling_rate = load_wav_to_torch(filename)
        save_dir = os.path.join(self._save_dir, f"speaker_{sid}/230717")
        ...
        _file = filename.split("/")[-1].split(".")[0]
        # spec_filename = os.path.join(save_dir, filename.replace(".flac", ".spec.pt"))
        # 각 Speaker별 폴더 생성
    ...
    def get_text(self, text):
        if self.cleaned_text:
            ...
            text_norm = text_to_sequence(text, self._cleaner, self._g2p)
...
class TextAudioSpeakerCollate:
    ...
    def __call__(self, batch):
        ...
        """
        # Right zero-pad all one-hot text sequences to max input length
        # TODO:: 왜 batch[0] 만 2차원이고 그 이후는 다 1차원인지 파악 필요
        _, ids_sorted_decreasing = torch.sort(torch.LongTensor([x[1].size(1) for x in batch]), dim=0, descending=True)
...
class DistributedBucketSampler(torch.utils.data.distributed.DistributedSampler):
    ...
    Ex) boundaries = [b1, b2, b3] -> any batch is included either {x | b1 < length(x) <=b2} or {x | b2 < length(x) <=
    b3}.
